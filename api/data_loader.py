...
class DataLoader:    
    ...
    def load(neo, data):
        ...
        query_builder\
            .merge().node("l", "Location", name=data["location"])\
                .merge().node("c", "Company", name=data["company"], logo=data["logo"])\
                    .merge().node("i", "Internship", name=data["title"], desc=data["description"], link=data["link"])\
        ...
        for index, skillset in enumerate(data["skills"]):
            ...
            query_builder\
                .merge().node(f"s{index}", "Skill", name=skill)\
                    .merge().node(f"t{index}", "Theme", name=theme)\
                        .merge().node(f"s{index}", "").relation(f"fits{index}" ,"FITS", Direction.NONE).node(f"t{index}", "")\
                            .merge().node(f"s{index}", "").relation(f"require{index}" ,"REQUIRE", Direction.NONE).node("i", "")
