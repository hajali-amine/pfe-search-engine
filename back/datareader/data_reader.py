...
class DataReader:
    ...
    def search_by_filter(neo, filter, search):
        ...
        query_builder.match().node("l", "Location").relation(
            "in", "IN", Direction.NONE
        ).node("i", "Internship").match().node("c", "Company").relation(
            ...
            "req", "REQUIRE", Direction.NONE
        ).node(
            "s", "Skill"
        ).where(
            f"toLower({filter[0]}.name) CONTAINS toLower('{search}')"
        ).to_return(
            "{internship: i.name, description:i.description, location: l.name, company: c.name, logo: c.logo, skills: COLLECT(s.name)}"
        )
    ...
    def search_by_skill(neo, search):
        ...
        query_builder.match().node("i", "Internship").relation(
            "req", "REQUIRE", Direction.NONE
        ).node("s", "Skill").match().node("l", "Location").relation(
            "in", "IN", Direction.NONE
        ).node(
            "i", "Internship"
        ).match().node(
            ...
            "req_prime", "REQUIRE", Direction.NONE
        ).node(
            "i", "Internship"
        ).where(
            f"toLower(s.name) CONTAINS toLower('{search}')"
        ).to_return(
        ...
        ).node("s", "Skill").match().node("related_skills", "Skill").relation(
            "related_fits", "FITS", Direction.NONE
        ).node(
            "t", "Theme"
        ).match().node(
            "i", "Internship"
